...
from firebase_functions import params, https_fn, options
import requests
import json
import os
import crypto
...
# Initialize Firebase Admin (prevents reinitialization)
...
def strava_auth_initiate(request):
    ...
    try:
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            return https_fn.Response(status=401, headers=headers, message="Unauthorized")
        ...
        if not authorization_code:
            return https_fn.Response(status=401, headers=headers, error="Authorization code required")
