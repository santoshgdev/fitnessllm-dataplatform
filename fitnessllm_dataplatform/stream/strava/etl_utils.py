...
def convert_stream_json_to_dataframe(
    ...
    module_strava_json_list = (
        list(
            itertools.islice(
                partial_strava_storage(strava_model=stream).iterdir(),
                int(environ["SAMPLE"]),
            )
        )
        if environ.get("SAMPLE")
        else list(partial_strava_storage(strava_model=stream).iterdir())
    )
...
def load_json_into_bq(
    ...
    for stream in streams:
        dataframes, metrics = convert_stream_json_to_dataframe(
            stream=stream,
            client=client,
            athlete_id=athlete_id,
            partial_strava_storage=partial_strava_storage,
        )
        upsert_to_bigquery(
            client=client,
            athlete_id=athlete_id,
