...
class Startup:
    ...
    def etl(self, data_source: str, athlete_id: str, data_streams: str | list[str] | None) -> None:
        """Entry point for loading JSONs into BigQuery."""

        if data_source == FitnessLLMDataSource.STRAVA.value:
            load_json_into_bq(InfrastructureNames=self.InfrastructureNames, athlete_id=athlete_id, data_streams=data_streams)
