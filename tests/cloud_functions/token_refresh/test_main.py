...
@patch('cloud_functions.token_refresh.main.partial_log_structured', new=lambda *a, **kw: None)
@patch('cloud_functions.token_refresh.main.strava_refresh_oauth_token', new=lambda *a, **kw: None)
    ...
    with patch('google.cloud.firestore.Client', return_value=db):
        ...
        with app.app_context():
            with app.test_request_context(
                ...
                assert "Token refreshed successfully for Strava" in json.loads(response.response[0])['message']


@patch('cloud_functions.token_refresh.main.partial_log_structured', new=lambda *a, **kw: None)
@patch('cloud_functions.token_refresh.main.strava_refresh_oauth_token', new=lambda *a, **kw: None)
@patch('firebase_admin.auth.verify_id_token')
def test_token_refresh_missing_data_source(    mock_verify,
    mock_request,
    mock_decoded_token):
    ...
    with patch('google.cloud.firestore.Client', return_value=db):
        ...
        with app.app_context():
            with app.test_request_context(
                ...
                assert "Required data_source parameter is missing" in json.loads(response.response[0])['message']


@patch('cloud_functions.token_refresh.main.partial_log_structured', new=lambda *a, **kw: None)
@patch('cloud_functions.token_refresh.main.strava_refresh_oauth_token', new=lambda *a, **kw: None)
@patch('firebase_admin.auth.verify_id_token')
def test_token_refresh_invalid_token(    mock_verify,
    mock_request,
    mock_decoded_token):
